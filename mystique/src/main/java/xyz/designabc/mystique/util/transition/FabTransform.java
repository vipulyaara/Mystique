...
public class FabTransform extends Transition {
    ...
    private ObjectAnimator iconFade;
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public FabTransform(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = null;
        try {
    ...
    public Animator createAnimator(@NonNull final ViewGroup sceneRoot,
        ...
        if (fabIcon != null) {
            iconFade = ObjectAnimator.ofInt(fabIcon, "alpha", fromFab ? 0 : 255);
        }
        if (!fromFab) {
