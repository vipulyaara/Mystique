...
public class CardTransform extends Transition {
    ...
    public CardTransform(@ColorInt int fabColor) {
        color = fabColor;
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public CardTransform(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = null;
        try {
    ...
    /**
     * Create a {@link CardTransform} from the supplied {@code activity} extras and set as its
    ...
    public static boolean setup(@NonNull Activity activity, @Nullable View target) {
        ...
        final CardTransform sharedEnter = new CardTransform(color);
        if (target != null) {
